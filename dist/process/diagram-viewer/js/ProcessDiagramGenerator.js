 ...
 var ProcessDiagramGenerator = {
 	...
 	initProcessDiagramCanvas: function(processDefinitionDiagramLayout) {
 		...
 		if (diagramCanvas) {
 			...
 			if (!diagramHolder)
 				throw {
 					msg: "Diagram holder not found",
 					error: "diagramHolderNotFound"
 				};
 	...
 	showFlowInfo: function(flow) {
 		...
 		var TPL_FLOW_INFO = '<div>{flow}</div>' + '<div><b>sourceActivityId</b>: {sourceActivityId}</div>' + '<div><b>destinationActivityId</b>: {destinationActivityId}</div>' + '<div><b>isDefault</b>: {isDefault}</div>' + '<div><b>isConditional</b>: {isConditional}</div>' + '<div><b>isHighLighted</b>: {isHighLighted}</div>';
 	...
 	showActivityInfo: function(activity) {
 		...
 		var TPL_ACTIVITY_INFO = '' + '<div><b>activityId</b>: {activityId}</div>' + '<div><b>name</b>: {name}</div>' + '<div><b>type</b>: {type}</div>';
 		var TPL_CALLACTIVITY_INFO = '' + '<div><b>collapsed</b>: {collapsed}</div>' + '<div><b>processDefinitonKey</b>: {processDefinitonKey}</div>';
 	...
 	showAudit_object: function(activity, processDefinitionId) {
 		if (activity.getProperty("type") != 'userTask') return;
 		parent.showAudit(processDefinitionId, activity.getId());
 		//$("#audit_object").panel('open',{cache:false,href:'./jsgl.htm?processUUID='+processDefinitionId+"&step="+activity.getId()});
 	},
 	hideInfo: function() {
 		var diagramInfo = $("#" + this.options.diagramInfoId);
 		if (!diagramInfo) return;
