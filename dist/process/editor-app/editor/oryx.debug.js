...
ORYX.Core.SVG.Label = Clazz.extend({
	...
	setEdgePosition: function(position) {
		if (["starttop", "startmiddle", "startbottom",
				"midtop", "midbottom", "endtop", "endbottom"
			].include(position)) {
	...
	getReferencedElementWidth: function() {
		...
		if (refNode) {
			...
			if (refbb) {
				return (this._rotate != 0 && this._rotate % 180 != 0 && this._rotate % 90 == 0 ?
					refbb.height : refbb.width);
			}
		}
	...
	/** @lends ORYX.Core.AbstractShape.prototype */
	{
		...
		construct: function(options, stencil, facade) {
			...
			this._stencil.properties().each((function(property) {
				...
				// 为了使overrideid初始化成resourceId -- by xyx ========
				// 原代码：
				// this.properties[key] = property.value();
				var value = property.value();
                var propName = value.match(/\${\S*?}/);
				while(propName){
                    value = value.replace(propName, this[propName[0].slice(2,-1)] || "");
                    propName = value.match(/\${\S*?}/);
                }
		...
		toJSON: function() {
			var json = {
				...
				properties: jQuery.extend({}, this.properties, this.hiddenProperties).inject({}, function(props, prop) {
					...
					if (this.getStencil().property(key) && this.getStencil().property(key).type() === ORYX.CONFIG.TYPE_COMPLEX && Object.prototype.toString.call(value) === "String") {
			...
			if (this.getIncomingShapes) {
				json.incoming = this.getIncomingShapes().map(function(shape) {
					return {
						resourceId: shape.resourceId
					};
				});
...
ORYX.Plugins.CanvasResizeButton = Clazz.extend({
	...
	construct: function(canvas, position, callback) {
		...
		var isOverOffset = function(event) {
			...
			var isOverButton = event.target.id.indexOf("canvas-shrink") != -1 || event.target.id.indexOf("canvas-grow") != -1 || event.target.parentNode.id.indexOf("canvas-shrink") != -1 || event.target.parentNode.id.indexOf("canvas-grow") != -1;
			if (isOverButton) {
	...
	checkRules: function(options) {
		...
		this.containmentParentNode = underlyingNodes.reverse().find((function(node) {
			return (node instanceof ORYX.Core.Canvas) ||
				(((node instanceof ORYX.Core.Node) || ((node instanceof ORYX.Core.Edge) && !noEdges)) && (!(this.currentShapes.member(node) ||
...
ORYX.Plugins.ShapeHighlighting = Clazz.extend({
	...
	setHighlight: function(options) {
		if (options && options.highlightId) {
			...
			if (!node) {
				node = ORYX.Editor.graft("http://www.w3.org/2000/svg", this.node, ['path', {
					"stroke-width": 2.0,
					"fill": "none"
				}]);
	...
	setAttributesByStyle: function(node, options) {
		...
		} else if (options.elements.length == 1 && options.elements[0] instanceof ORYX.Core.Edge &&
