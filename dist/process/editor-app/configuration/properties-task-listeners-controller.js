...
var KisBpmTaskListenersPopupCtrl = ['$scope', '$q', '$translate', function($scope, $q, $translate) {
    ...
    if ($scope.property.value !== undefined && $scope.property.value !== null && $scope.property.value.taskListeners !== undefined && $scope.property.value.taskListeners !== null) {
    ...
    $q.all([eventPromise, implementationPromise, namePromise]).then(function(results) {
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [{
                field: 'event',
                displayName: $scope.labels.eventLabel
            }, {
                field: 'implementation',
                displayName: $scope.labels.implementationLabel
            }]
        };
        ...
        $scope.gridFieldOptions = {
            ...
            columnDefs: [{
                field: 'name',
                displayName: $scope.labels.nameLabel
            }, {
                field: 'implementation',
                displayName: $scope.labels.implementationLabel
            }]
    ...
    $scope.listenerDetailsChanged = function() {
        var selected = $scope.selectedListeners[0];
        if (selected.className !== '') {
            selected.implementation = selected.className;
        } else if (selected.expression !== '') {
            selected.implementation = selected.expression;
        } else if (selected.delegateExpression !== '') {
            selected.implementation = selected.delegateExpression;
        } else {
    ...
    $scope.addNewListener = function() {
        $scope.taskListeners.push({
            event: 'create',
            implementation: 'com.websuites.esb.kbpm.listener.UserTaskSetUserListenerImpl',
            className: 'com.websuites.esb.kbpm.listener.UserTaskSetUserListenerImpl',
            expression: '',
            delegateExpression: ''
        });
    };
