...
var App = function() {
    ...
    return {
        ...
        //     });
        // },
        initPagination: function(cp, tp, tr, id, size, cfunc, isChangeSize, isGoPage) { //当前页，总页码，总条数，分页id, 回调， 是否显示改变每页条数， 是否显示页数跳转
            if (arguments.length == 1) {
                var id = arguments[0];
            }
            $.kd.kdPager.init({
        ...
        initCheckableTable: function(table) {
            var table = table;
            this.initUniform();
        ...
        //datepicker
        handleDatePickers: function() { //初始化datepicker
        ...
        // 获取表格数据
        getTableParamsFakeName: selector => {
            let arr = [],
            ...
            $.each($(`${selector} tr:not([hidden])`), function() {
                ...
                $.each(_this.find("[fakename]"), function(i) {
                    let dom = _this.find("[fakename]").eq(i),
                        name = dom.attr("fakename"),  // 为什么用fakename，避免getFormParams方法获取了name
                        selectTrue = dom.prop("tagName") === "SELECT", //是否为select标签
        ...
        // 获取局部表单
        getFormParamsFakeName: selector => { 
            let obj = {};
            $.each($(`${selector} .form-control`), function() {
                let name = $(this).attr("fakename"),
                    value = $(this).val();
                obj[name] = value;
            });
            return obj;
        ...
        handleParams: function(selector) {
            this.addRow = () => {
            ...
            this.getData = cb => {
                let parameters = App.getTableParamsFakeName(selector);
                ...
                for (let item of items) {
                    ...
                    $.each(clone.find("[fakename]"), function(i) {
                        ...
                        if (fakename === "value" && item.direction === "IN") {
                            $(this).attr("required", "required").attr("placeholder", "这是必填参数");
                        }
                    });
                    $(selector).append(clone);
                }
        ...
        treeEllipsis: function(treeSelector) {
            ...
            $.each($(`${treeSelector} .jstree-anchor`), function() {
                ...
                let temp = $(this).children("i").prop("outerHTML");
                const [ len, index ] = getRealLength(title);
                if (len > 16) {
        ...
        treeEllipsis1: function(treeSelector) {
            function isChinese(str) {
                let reg = /[\u4e00-\u9fa5]/;
                if (reg.test(str)) {
                    return true;
        ...
        checkDate: function(selector, lable="") {
            let start = Date.parse(selector.find(".date-time-picker,.date-picker").first().val()),
