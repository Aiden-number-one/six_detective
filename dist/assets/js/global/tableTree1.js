 ...
 * @Description: 表格树形
 */
function TableTree1(element, options = {}) {
...
TableTree1.prototype.init = function() {
    let html = this.generateTableTree1("id", "pId", this.options.data);
    ...
    for (let item of data) {
        ...
        for (let i in item.columns) {
            if (!this.isTheadRendered) {
                this.ths.push(`<th class="t-l" style="min-width:40px"> ${i} </th>`);
            }
...
TableTree1.prototype.openNode = function(key, isChangeIcon = true) {
    let node = $(`[key="${key}"]`); // tr
...
TableTree1.prototype.closeNode = function(key, isChangeIcon = true) {
    let node = $(`[key="${key}"]`); // tr
    let childNode = $(`[pkey="${key}"]`); // 子集tr
    let _this = this;
    childNode.hide();
