 ...
 * @LastEditTime : 2020-01-22 14:19:55
...
class InsertTag {
  apply(compiler) {
    ...
    compiler.hooks.emit.tapAsync('InsertTag', (compilation, callback) => {
      ...
      const regExp = /^xspreadsheet\..{6}\.js$/;
      const dir = path.join(__dirname, '../public/xspreadsheet');
      const fileList = fs.readdirSync(dir); // 读取目录
      const targetName = fileList.filter(v => regExp.test(v))[0]; // 匹配到要插入的文件名

      if (targetName) {
        const ae = compilation.getAsset(targetName);
        console.log('ae -> ', ae);
      }
