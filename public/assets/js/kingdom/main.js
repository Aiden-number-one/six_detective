...
define(function (require, exports, module) {
    ...
    require("js/kingdom/polyfill.min.js");
    ...
    /**页面跳转**/
    var jumpToLink = function jumpToLink(link, tag) {
        //首页
        if (!link) {
            link = "index";
        }
    ...
    var rewriteBreadcrumb = function rewriteBreadcrumb(link) {
    ...
    var createMenu = function createMenu(cbfunc) {
        var api_params = {
    ...
    // 高亮导航栏
    var refreshSidebar = function refreshSidebar(link) {
    ...
    var init = function init() {
        ...
        $(window).bind('hashchange', function () {
            ...
            if (location.hash !== "#report-table-info-manage") {
                ...
                var newHref = oldHref.replace(location.search, "");
        ...
        $('body').on('click', '.nav-link', function () {
            ...
            var isSameHash = location.hash === href;
    ...
    //修改密码
    var setDistributorLoginPwd1 = function setDistributorLoginPwd1(params) {
    ...
    var slideToggle = function slideToggle(page) {
        if ($(page + " .slide-toggle").length === 0) {
            return;
        }
        var FULL_WIDTH = $("body").width();
        var L_WIDTH = $(page + " .left-side").width();
        var S_WIDTH = $(".page-logo").outerWidth();
        var LEFT = $(page + " .slide-toggle").position().left;
    ...
    /**
         ...
         */
    var slideToggleTreeCommon = function slideToggleTreeCommon(page) {
        ...
        // var S_WIDTH = $(".page-logo").outerWidth();
        $(page + " .right-content").css("width", "calc(100% - " + (L_WIDTH + 10) + "px)");
        var LEFT = $(page + " .drag_bar").position().left;
    ...
    function initDragTable() {
        ...
        $('body').on('mousedown', ".movableLine", function (e) {
            ...
            function onMouseUp(e) {
                ...
                var width = $(this).parent("th").width();
                var clientX = e.clientX;
                var offsetX = clientXBefore - clientX;
                $(this).parent("th").width(width - offsetX);
                $(this).css("right", 0);
            }
        });
    }
    ...
    $(function () {
        ...
        $("body").on("click", ".sidebar-toggler", function () {
            var page = ".page-" + location.hash.substring("1");
            var width = $(page + " .right-content").width();
            var L_WIDTH = $(page + " .left-side").width();
        ...
        (function getCalendar() {
            ...
            function displayDetail(event) {
                ...
                require.async("../../../applications/common-template/calendar-datail-list.handlebars", function (compiled) {
        ...
        $("body").on('click', '#J_variable_modal .variable-r .list-group-item .v-close', function () {
            var variablename = $(this).closest(".list-group-item").data("variablename");
        ...
        $("body").on('change', '#J_variable_table tbody tr td input[type=checkbox]', function () {
            ...
            if (!checked) {
                $.each(choose, function () {
                    var variableName_ = $(this).data("variablename");
        ...
        $("body").on('click', '#J_choose_variable', function () {
            var choose = $("#J_variable_modal .variable-r .list-group-item:not('.first')");
            if (choose.length === 0) {
                toastr.info("请选择变量");
                return;
            }
        ...
        //  查询变量
        var getVariable = function getVariable(params) {
            var _params = {
                taskType: "TV"
            }; // 这里用来传特殊参数
