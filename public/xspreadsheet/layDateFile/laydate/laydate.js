    ...
    ready = {
      ...
      link: function(href, fn, cssname) {
        ...
        (function poll() {
          ...
          parseInt(ready.getStyle(document.getElementById(id), 'width')) === 1989
            ? fn()
            : setTimeout(poll, 100);
        })();
      },
    ...
    laydate = {
      ...
      ready: function(fn) {
        var cssname = 'laydate',
          ver = '',
          path =
            (isLayui ? 'modules/laydate/' : 'theme/') + 'default/laydate.css?v=' + laydate.v + ver;
  ...
  */
    ...
    var that = this,
      ...
      elem = (that.elem = lay.elem('div', {
        ...
        class: [
          ...
          options.theme && options.theme !== 'default' && !/^#/.test(options.theme)
    ...
    lay(divFooter).html(
      (function() {
        ...
        if (options.type === 'datetime') {
          html.push(
            '<span lay-type="datetime" class="laydate-btns-time">' + lang.timeTips + '</span>',
          );
        }
        lay.each(options.btns, function(i, item) {
          ...
          btns.push(
            '<span lay-type="' + item + '" class="laydate-btns-' + item + '">' + title + '</span>',
          );
        });
  ...
  Class.prototype.position = function() {
    ...
    that.elem.style.left =
      left - options.colWidth.width + (options.position === 'fixed' ? 0 : scrollArea(1)) + 'px';
  ...
  Class.prototype.stampRange = function() {
    ...
    lay.each(tds, function(i, item) {
      ...
      if (thisTime === startTime || thisTime === endTime) {
        lay(item).addClass(
          lay(item).hasClass(ELEM_PREV) || lay(item).hasClass(ELEM_NEXT) ? ELEM_SELECTED : THIS,
        );
      }
      if (thisTime > startTime && thisTime < endTime) {
    ...
    var that = this,
      ...
      showEvent = function(elem, bind) {
        elem.on(options.trigger, function() {
          ...
          // linjian 因为在一个元素上多次执行render方法，传入参数不一样，但是始终是第一个参数生效，所以这里先执行屏蔽
          // that.render();
        });
      };
