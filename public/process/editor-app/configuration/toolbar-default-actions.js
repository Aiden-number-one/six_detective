...
KISBPM.TOOLBAR = {
  ACTIONS: {
    ...
    alignVertical: function(services) {
      KISBPM.TOOLBAR.ACTIONS._getOryxArrangmentPlugin(services.$scope).alignShapes([
    ...
    var judgeJson = function(json) {
        ...
        onSuccess: function(data) {
          ...
          jQuery.each(stencilsList['BPMNDiagram'].hiddenPropertyPackages, function(index, v) {
            ...
            if (v == 'process_idpackage' && !json.properties[propertyPackagesList[v].id]) {
              propertyPackagesList[v].value =
                propertyPackagesList[v].value + '_' + new Date().getTime();
      ...
      for (i = 0; i < childShapesForExclusive.length; i++) {
        ...
        if (UserTaskType !== 'UserTask') {
          ...
          incomingNodeId =
            childShapesForKey[childShapesForKey[incomingFlow[0].resourceId].incoming[0].resourceId]
              .resourceId;
