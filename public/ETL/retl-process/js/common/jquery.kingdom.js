        ...
        kingdom: {
            ...
            doKoauthAdminAPI: function(api_name, api_version, api_params, cbfunc, method) {
                    ...
                    $.ajax({
                        ...
                        timeout: 120000,
                        async: true,
                        beforeSend: function(request) {                            const randowNVPS = $.kingdom.getRandowNVPS();
            ...
            // 普通列表查询封装, 避免搬砖时复制一大堆
            getList: function(obj, cb) {
                let { apiName, apiVision, params, tableId, pageId, template, formId } = obj;
                var paramsMap = Object.assign({
                    "pageNumber": $(`#${pageId} input[name='pager_number']`).val() || "1",
                    "pageSize": "3"
                }, params);
                if (formId) paramsMap = Object.assign(App.getFormParams(formId), paramsMap);
            ...
            getDict: (dictType, selector, template) => {
                ...
                $.kingdom.doKoauthAdminAPI("bayconnect.superlop.get_all_dict_data_list", "v4.0", {}, data => {
                    if (data.bcjson.flag == "1") {
                        ...
                        let arr = [];
                        for (let i in items[dictType]) {
                            let obj = {
                                id: i,
                                text: items[dictType][i]
                            }
                            arr.push(obj);
                        }
                        // 渲染数据
