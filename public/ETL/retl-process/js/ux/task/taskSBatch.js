...
//保存数据-数据抽取
function saveSBacth(){
   if ($("#formSBatch").form('validate')) {//启用校验
	   var rows = document.getElementById("btn-tb1").rows.length ;
	   for(var i=0;i<rows-1;i++){
		...
		var data=$('#formSBatch').serialize();
		if(data == null || data == ''){
			$.messager.alert('提示', "请添加待保存任务信息.", 'waring');
		}else{
			$.ajax({  
				type: "POST",  
				url: "task/saveSBatch.action",  
				data: data,  
...
function addLine(i,data){
	...
	html+=	   "<td width=\"10%\">" +
	    "<input class=\"hidden\" type=\"text\" name=\"command\" id=\"commandx"+i+"\" value=\"0\"> "+
			"<textarea class=\"form-control text_control\"  id=\"command"+i+"\" rows=\"3\" style=\"width:260px;height:25px;\"></textarea>" +
		"</td>";
	//成功返回值
	html+=	   "<td width=\"10%\">" +
			"<input name=\"successFlag\" id=\"successFlag"+i+"\" class=\"easyui-validatebox Task-number form-control\" type=\"text\" style=\"width:160px;\"  value=\"0\""+
			"</td>";
	//返回值变量赋值
	html+=	   "<td width=\"10%\">" +
			"<input name=\"returnVariable\" id=\"returnVariable"+i+"\" class=\"easyui-validatebox Task-number form-control\" type=\"text\" style=\"width:220px;\" "+
			"</td>";
