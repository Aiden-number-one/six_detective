				...
				if (metadata.flag) {
					...
					for (var i =0; i <dlength; i++) {
						...
						if (i == 0) {
							...
							// 与子分类列匹配
							metadata.quarter
									.push(data.data[i - 1] === data.data[i] ? ''
											: data.data[i]);
						}
						...
						metadata.x_major_offset = metadata.x_major_offset > data.data[i].length ? metadata.x_major_offset
								: data.data[i].length;
					}
					metadata.flag = false;
				}
		...
		var option = {
			tooltip : {
				...
				formatter : function(params) {
					var res = '【' + metadata.init().jobNos[params.dataIndex]
							+ '】' + metadata.init().jobNames[params.dataIndex]
							+ '<br/>' + params.seriesName + '<br/>'
...
//			startInfo = dlength;
		...
		myChart.on('click',function(param) {
				...
				if (typeof param.seriesIndex != 'undefined') {
					...
					// 查询日志信息
					$("#jkjginfo1").html('作业名称：【'
											+ metadata.init().jobNos[param.dataIndex]
											+ '】'
											+ metadata.init().jobNames[param.dataIndex])
...
function init_graph(executorId, batchNo) {
	...
	$.ajax({
		type : "get",
		url : "tbDiMonitorLog/getMonitorXML.action?executorId=" + executorId
				+ "&batchNo=" + batchNo,
...
//	Peony.progress();
	...
	if (scheduleId != "" || batchNo != "" || executorId != ""
			|| executeType != "" || executeFlag != "") {
...
function initDataGrid(url) {
	$('#data_list').datagrid({
		...
		rowStyler : function(index, row) {
			...
			if (executeFlag == 'F' || executeFlag == 'B'
					|| executeFlag == 'A' || executeFlag == 'I') {
