...
//	console.log(_array);
	...
	for (var i = 0; i < _array.length - 1; i++) {
		if (_array[i] == _array[i + 1]) {
			Peony.alert('提示', '存在重复节点编号 "' + _array[i] + '"，请修改后再提交！',
			'error');
			flag = false;
			break;
		}
...
function btnAddCondition() {
	...
	$('#delAllLineC').unbind('click').click(function() {// 删除全部
...
function addLineCondition(i, data) {
	...
	html += '  <td><input type="text" class="form-control" name="i_variable_name" id="i_variable_name'
			+ i + '"></td>';
	html += '  <td><select name="i_variable_type" class="form-control" id="i_variable_type'
			+ i + '">';
	...
	html += '    </select></td>';
	html += '  <td><input type="text" class="form-control" id="i_variable_value'
			+ i + '" name="i_variable_value"></td>';
	html += '  <td><select name="i_operation" class="form-control" id="i_operation'
			+ i + '">';
	html += '      <option value="1">AND</option>';
	html += '      <option value="2">OR</option>';
	html += '    </select></td>';
	html += '  <td width="10%"><a class="easyui-linkbutton remove-btn" iconCls="icon-remove" plain="true"></a></td>';
	html += '</tr>';
	var line = $(html);
	// 版定删除按钮事件
