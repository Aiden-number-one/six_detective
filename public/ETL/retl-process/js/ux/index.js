			...
			if (data.data != null && dlength > 0) {
				...
				myChart.on('click', function (param) {
					...
					if (typeof param.seriesIndex != 'undefined') {
						...
						addTab("柱形图信息", getRealPath() + "/tbDiMonitorLog/imageInfo.action?scheduleId=" + sid + "&batchNo=" + param.name + "&executorId=" + cloudid + "&executeFlag=" + eflag);
	...
	$('#data-list').datagrid({
		...
		columns: [[{
			...
			formatter: function formatter(value, row, index) {
				if (row.startFlagName == '开启') return '<strong><font color="green">开启</font></strong>';else return row.startFlagName;
			}
		}]],
