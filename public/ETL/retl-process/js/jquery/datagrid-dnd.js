(function ($) {
	$.extend($.fn.datagrid.defaults, {
		...
		dragSelection: false,
		dragDelay: 100,
		onBeforeDrag: function onBeforeDrag(row) {}, // return false to deny drag
		onStartDrag: function onStartDrag(row) {},
		onStopDrag: function onStopDrag(row) {},
		onDragEnter: function onDragEnter(targetRow, sourceRow) {}, // return false to deny drop
		onDragOver: function onDragOver(targetRow, sourceRow) {}, // return false to deny drop
	...
	$.extend($.fn.datagrid.methods, {
		_appendRows: function _appendRows(jq, row) {
		...
		_insertRows: function _insertRows(jq, param) {
			return jq.each(function () {
				var dg = $(this);
		...
		_getRowIndexs: function _getRowIndexs(jq, row) {
			var dg = jq;
			var rows = $.isArray(row) ? row : [row];
		...
		_deleteRows: function _deleteRows(jq, indexs) {
			return jq.each(function () {
				// sort desc
		...
		_setSelections: function _setSelections(jq) {
			return jq.each(function () {
		...
		clearInsertingFlag: function clearInsertingFlag(jq) {
			return jq.each(function () {
	...
	$.extend($.fn.datagrid.methods, {
		resetDroppable: function resetDroppable(jq) {
			return jq.each(function () {
		...
		enableDnd: function enableDnd(jq, index) {
			if (!$('#datagrid-dnd-style').length) {
				$('<style id="datagrid-dnd-style">' + '.datagrid-row-top>td{border-top:1px solid red}' + '.datagrid-row-bottom>td{border-bottom:1px solid red}' + '</style>').appendTo('head');
			}
			return jq.each(function () {
				var target = this;
