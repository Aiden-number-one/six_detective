...
$(document).ready(function () {
	...
	var logShow = false;
	var logout = function logout(cbfunc) {
	...
	var kd = {
		menus: [],
		generateTreeData: function generateTreeData(id, arry, name, attrName) {
		...
		// id：判断是否有下级菜单的标识
		// arry：菜单数组信息
		getChildArry: function getChildArry(id, arry, name) {
...
$('#saveInfo').click(function () {
	...
	if (dlist != null && dlist.rows.length > 0) {
		...
		try {
			for (var _iterator = job_module[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
...
function getTask(params) {
	$.kingdom.doKoauthAdminAPI("bayconnect.superlop.get_task_info", "v4.0", params, function (data) {
		if (data.bcjson.flag == "1") {
			var items = data.bcjson.items;
