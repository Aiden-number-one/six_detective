define(function(require, exports, module) {
    ...
    showContent._load = function() {
        ...
        if (params) {
         ...
         // 顶部铃铛跳转过来
        //  isJump判断是否是从铃铛跳转过来
         let isJump = sessionStorage.getItem("isJump");
        ...
        (function getCurrentTimeLog() {      
            $.kingdom.doKoauthAdminAPI("kingdom.retl.get_sys_current_date", "v4.0", {}, function(data) {
                if (data.bcjson.flag === "1" && data.bcjson.items[0]) {
                    let times = data.bcjson.items[0].currentDate;
                    let startDate = times.substring(0,8);
                    let toDay =  startDate.substring(0,4)+"-"+startDate.substring(4,6)+"-"+startDate.substring(6,8);                  
    ...
    showContent.getWorkChart = function(params) {
        $.kingdom.doKoauthAdminAPI(
            "kingdom.retl.get_all_monitor_log_info",
            "v4.0",
            params,
            function(data) {
                ...
                if (data.bcjson.flag == "1" && items && items.length > 0) {
                    ...
                    for (let item of items) {
                        showContent.jobNameArr.push('【' + item.batchNo + '】' + item.jobName);
                        xAxis.push($.kingdom.toDateTime(item.endTime));
                        if (flagArr[item.executeFlag]) {
                            for (let serie of series) {
                                if (item.executeFlag == serie.executeFlag) {
                                    serie.data.push((item.duration / 1000) || 0);
                                } else {
                                    serie.data.push("-");
                                }
                            }
    ...
    showContent.getWorkChart2 = function(params) {
        $.kingdom.doKoauthAdminAPI(
            "kingdom.retl.get_current_job_monitor_log_info",
            "v4.0",
            params,
            function(data) {
                var items = data.bcjson.items || data.bcjson;
    ...
    showContent.getLogBatchList = function(params) {
        ...
        let calendarJumpParams = sessionStorage.getItem("calendarJumpParams");
    ...
    showContent.displayDetail = function(event) {
      ...
      $("#J_lq_tab2_query_form input[name=endDate]").val(endTime);  
        ...
        let calendarJumpParams =$.extend({"executeFlagName":event.currentAllData[0].executeFlagName},{"executeType":"A"});
 ...
 //获取大框table的数据
    showContent.getTableData = function(params) {
        ...
        var paramsMap = $.extend({}, params);
        $.kingdom.doKoauthAdminAPI("kingdom.retl.get_job_detail_flow_info", "v4.0", paramsMap, function(data) {
            // console.log(data);
            var items = data.bcjson.items || data.bcjson;
            if (data.bcjson.flag == "1") {
