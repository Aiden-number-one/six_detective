...
define(function(require, exports, module) {
    ...
    showContent.addMyTree = function (params) {
        ...
        $.kingdom.doKoauthAdminAPI(
            "kingdom.retl.set_my_focus_job_modify",
            "v4.0",
            paramsMap,
            data => {
                App.unblockUI();
        ...
        $.kingdom.doKoauthAdminAPI(
            ...
            function gene(items) {
              ...
              showContent.getLeftBoxList = function(params, isSearch) {
                $.kingdom.doKoauthAdminAPI("kingdom.retl.get_all_job_deal_info", "v4.0", params ? params : {}, function(data) {
            ...
            showContent.getRightTopBoxList = function(params) {
                ...
                $.kingdom.doKoauthAdminAPI("kingdom.retl.get_job_finish_rate_info", "v4.0", paramsMap, function(data) {
                    var items = data.bcjson.items || data.bcjson;
                    if (data.bcjson.flag === "1" && items.length > 0) {
                        //保存当前任务的jobId
                        var currentJobId = {};
            ...
            showContent.getPieList = function(paramsMap) {
                ...
                $.kingdom.doKoauthAdminAPI("kingdom.retl.get_each_batch_finish_data_info", "v4.0", paramsMap, function(data) {
                    if (data.bcjson.flag == 1) {
                        var items = data.bcjson.items || data.bcjson;
	...
	showContent.variableSetting = function (params) {
		$.kingdom.doKoauthAdminAPI("kingdom.retl.get_job_task_var_list", "v4.0", params, function (data) {
			var items = data.bcjson.items || data.bcjson;
			if (data.bcjson.flag === "1") {
				let html_t = ``;
	...
	showContent.selectTask = function (params) {
		...
		var paramsMap = $.extend({}, params);
		$.kingdom.doKoauthAdminAPI("kingdom.retl.get_job_xml_nodeinfo", "v4.0", paramsMap, function (data) {
	...
	showContent.performTask = function (params) {
		...
		var paramsMap = $.extend({}, params);
		$.kingdom.doKoauthAdminAPI("kingdom.retl.set_job_manual_exec", "v4.0", paramsMap, function (data) {
			var items = data.bcjson.items || data.bcjson;
			if (data.bcjson.flag == "1" && items) {
				//得到新的batchNo
	...
	showContent.stopCurrentTask = function (jobId, batchNo) {
		...
		$.kingdom.doKoauthAdminAPI("kingdom.retl.set_job_manual_break", "v4.0", params, function (data) {
			App.unblockUI();
			if (data.bcjson.flag === "1") {
