...
define(function (require, exports, module) {
    ...
    showContent.addMyTree = function (params) {
        ...
        $.kingdom.doKoauthAdminAPI("bayconnect.superlop.set_my_focus_job_modify", "v4.0", paramsMap, function (data) {
            App.unblockUI();
            ...
            if (data.bcjson.flag == "1") {
                if (checkIsReloadLeftList(showContent.preLeftBoxItems, items)) {
                    if (items.length > 0) {
                        ...
                        var treeData = [{ text: "Error Interrupt", typeFlag: "B", icon: "fa fa-times-circle icon-state-danger", children: [] }, { text: "Executing", typeFlag: "R", icon: "fa fa-spinner icon-state-info", children: [], state: { opened: true } }, { text: "Succeed", typeFlag: "S", icon: "fa fa-check-circle icon-state-success", children: [] }, { text: "Error completed", typeFlag: "F", icon: "fa fa-times-circle icon-state-warning", children: [] }, { text: "Create", typeFlag: "N", icon: "fa fa-plus-circle icon-state-default", children: [] }];
                ...
                require.async("./template/perform-rightMiddle-round.handlebars", function (compiled) {
                    ...
                    if (items && items.length > 0) {
                        $.each(items, function (index, item) {
                            ...
                            var data;
                            if (taskSum) {
                                data = [{ value: initail, name: 'Unexecuted' }, { value: successNum, name: 'Success' }, { value: errorNum, name: 'Failure' }];
                            } else {
                                data = [{ value: 0, name: 'Executing' }];
                            }
                            var params = {
    ...
    showContent.createPieChart = function (data, index, params) {
        if (index == "0") {
            ...
            $('#circle' + index).siblings(".circleChart-text").css("margin-left", "65px").siblings(".num-block").css("width", "120%");
        }
            ...
            if (data.bcjson.flag === "1") {
                ...
                try {
                    for (var _iterator5 = items[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
                        ...
                        _item.no = ++i;
                        html_t += "<option value=\"" + _item.varName + "\">" + _item.varName + "</option>";
                    }
                } catch (err) {
    ...
    function checkIsReloadLeftList(preItems, newItems) {
        ...
        if (preItems.length !== newItems.length) {
            ...
            for (var i = 0, length = preItems.length; i < length; i++) {
                if (preItems[i]["jobName"] !== newItems[i]["jobName"] || preItems[i]["executeFlag"] !== newItems[i]["executeFlag"]) {
                    needReold = true;
                    return needReold;
                }
            }
