...
define(function (require, exports, module) {
    ...
    $(function () {
        ...
        $("body").on("click", "#data-source-configuration-query", function () {
            var params = App.getFormParams("data-source-configuration-form");
        ...
        $("body").on("click", "#data-source-configuration .data-source-configuration-list li", function () {
            ...
            $(this).siblings().removeClass("click-add-background");
        ...
        $("body").on("blur", "#data-connect-add-form input[name='principal']", function () {
            // 获取值时会重复添加principal=
            var arr = $("#J_dsc_commonType_add input[name=jdbc_string]").val().split(";principal=");
            var val = "";
            if (arr.length == 2) {
                val = arr[0];
            } else {
        ...
        $("body").on("click", "#data-source-configuration #test-data-connect", function () {
            if (!$("#data-connect-add-form").validate().form()) {
                return false;
            }
            var params = {};
            params.connection_name = $("#connection_name").val();
            params.connection_desc = $("#data-connect-add-form input[name='connection_desc']").val();
            ...
            params.db_database = $("#data-connect-add-form input[name='db_database']").val();
            params.db_user = $("#data-connect-add-form input[name='db_user']").val();
            var db_password = $("#data-connect-add-form input[name='db_password']").val();

            params.db_password = $.des.getDes(db_password);

            params.jdbc_string = $("#data-connect-add-form input[name='jdbc_string']").val();
            params.max_connect_count = $("#data-connect-add-form input[name='max_connect_count']").val();
            params.driverInfo = $("#connection_type option:selected").attr("className");
            App.blockUI({
                boxed: true,
                message: "Testing..."
            });
        ...
        $("body").on("click", "#data-source-configuration #data-connect-add-submit", function () {
            ...
            params.db_password = $.des.getDes($("#data-connect-add-form input[name='db_password']").val());
            params.jdbc_string = $("#data-connect-add-form input[name='jdbc_string']").val();
            params.max_connect_count = $("#data-connect-add-form input[name='max_connect_count']").val();
            params.driverInfo = $("#connection_type option:selected").attr("className");
            App.blockUI({
        ...
        $("body").on("click", "#data-source-configuration .data-source-configuration-list li a", function (e) {
            ...
            $("#data-connect-edit-form input[name='connection_desc']").val(data.connection_desc);
            $("#data-connect-edit-form input[name='server']").val(data.server);
            $("#data-connect-edit-form input[name='db_port']").val(data.db_port);
            $("#data-connect-edit-form input[name='db_database']").val(data.db_database);
            $("#data-connect-edit-form input[name='db_user']").val(data.db_user);
            $("#data-connect-edit-form input[name='db_password']").val(data.db_password);
            $("#data-connect-edit-form input[name='jdbc_string']").val(data.jdbc_string);
            $("#data-connect-edit-form input[name='max_connect_count']").val(data.max_connect_count);
        ...
        $("body").on("click", "#data-source-configuration #data-connect-edit-submit", function () {
            if (!$("#data-connect-edit-form").validate().form()) {
                return false;
            }
            ...
            params.connection_desc = $("#data-connect-edit-form input[name='connection_desc']").val();
            ...
            params.db_database = $("#data-connect-edit-form input[name='db_database']").val();
            params.db_user = $("#data-connect-edit-form input[name='db_user']").val();
            params.jdbc_string = $("#data-connect-edit-form input[name='jdbc_string']").val();
            params.max_connect_count = $("#data-connect-edit-form input[name='max_connect_count']").val();
            params.driverInfo = $("#connection_type_edit option:selected").attr("className");
            if (showContent.db_password && showContent.db_password == $("#data-connect-edit-form input[name='db_password']").val()) {
                params.db_password = showContent.db_password;
            } else {
                params.db_password = $.des.getDes($("#data-connect-edit-form input[name='db_password']").val());
            }
            App.blockUI({
                boxed: true,
                message: "modifying..."
        ...
        // 删除数据源
        $("body").on("click", "#data-source-configuration .data-source-configuration-list li .delBtn", function () {
            var params = {};
        ...
        //修改数据库/模式值
        $("body").on("change", "#data-source-configuration #data-connect-edit-form input[name='db_database']", function () {
        ...
        $("body").on("change", "#data-source-configuration #connection_type", function () {
            ...
            //     .attr("urlinfo");
            var driverid = $(this).find("option:selected").attr("driverid");
            var drivername = $(this).find("option:selected").attr("drivername");
        ...
        $("body").on("change", "#data-source-configuration #connection_type_edit", function () {
            var urlInfo = $(this).find("option:selected").attr("urlinfo");
            var driverid = $(this).find("option:selected").attr("driverid");
            var drivername = $(this).find("option:selected").attr("drivername");
            if (drivername === "JTDS SQL Server") {
                $("#data-connect-edit-form #db_password").attr("disabled", "disabled");
        ...
        $("body").on("blur", "#data-source-configuration #data-connect-add-form input[name='server'],#data-connect-add-form input[name='db_port'],#data-connect-add-form input[name='db_database']", function () {
            ...
            var val = $("#J_dsc_commonType_add input[name=principal]").val();
            // 当输入框为空时清除principal=
            if (val != "") {
        ...
        $("body").on("click", "#J_list_export", function () {
            ...
            showContent.listImport({
                ...
                return showContent.listImport3({
                    fileClass: data.bcjson.items[0].fileClass,
                    filePath: data.bcjson.items[0].filePath
                    // fileClass: "ZIP",
        ...
        $("body").on("change", "#J_select2_dcs_target_data_connect", function (e) {
            ...
            }, "#J_select2_des_target_object", false, function () {
                if (showContent.targetTable) {
                    // 取对应ID ,因为接口存的表名
                    var origin = $("#J_select2_des_target_object").data("origin");
        ...
        $("body").on("click", "#data-source-configuration #J_excel_add_tab3_btn [data-type]", function () {
            ...
            var table_id = $("#J_excel_add_tab2_form select[name='targetTable']").val();
            if (type === "refresh") {
                // 刷新
        ...
        $("body").on("click", "#data-source-configuration #J_dsc_next_step", function () {
            ...
            //第二步
            params = _extends(params, App.getFormParams('J_excel_add_tab2_form'));
