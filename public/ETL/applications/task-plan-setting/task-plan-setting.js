    ...
    $(function () {
        ...
        $("body").on("click", "#J_delete_btn", function () {
            ...
            $.each(checkDom, function (index, value) {
                if ($(value).closest("tr").data("scheduleid")) {
                    checkArray.push($(value).closest("tr").data("scheduleid"));
                }
            });
            var params = {};
        ...
        $("body").on("click", "#J_form_basc_submit_edit", function () {
            if ($('#J_form_basc').valid()) {
                var paramsMap = App.getFormParams("J_form_basc");
        ...
        $("body").on("click", "#J_form_basc_submit_add", function () {
            if ($('#J_form_basc').valid()) {
                ...
                if ($("#J_form_basc [fakename=forever]:checked").length === 1) {
                    ...
                    paramsMap.validStartDate = $.kingdom.timeFormat(new Date(year - 1 + "-" + month + "-" + date + " " + hour + ":" + minute + ":" + second).getTime()).replace(/[-: ]/g, "");
                    paramsMap.validEndDate = "20991231235959";
                }
                if (parseInt(paramsMap.validStartDate) > parseInt(paramsMap.validEndDate)) {
                    toastr.info("Start time should be earlier than end time");
        ...
        //选择秒、分、时 radio
        $("body").on("click", "#tab_s .line input[type='radio']," + "#tab_m .line input[type='radio']," + "#tab_h .line input[type='radio']," + "#tab_d .line input[type='radio']," + "#tab_w .line input[type='radio']," + "#tab_o .line input[type='radio']," + "#tab_y .line input[type='radio']", function () {
            ...
            if (index === 0 && inputIndex < 6) {
                ...
                towInput = $("#" + tabid + " .line:eq(1) input:eq(1)").val() + "-" + $("#" + tabid + " .line:eq(1) input:eq(2)").val();
                $("#crons_set_table input:eq(" + inputIndex + ")").val(towInput);
            } else if (index === 2 && inputIndex < 5) {
                var threeInput = $("#" + tabid + " .line:eq(2) input:eq(1)").val() + "/" + $("#" + tabid + " .line:eq(2) input:eq(2)").val();
                ...
                var fiveInput = $("#" + tabid + " .line:eq(3) input:eq(1)").val() + "W";
                ...
                towInput = $("#" + tabid + " .line:eq(2) input:eq(1)").val() + "#" + $("#" + tabid + " .line:eq(2) input:eq(2)").val();
                $("#crons_set_table input:eq(" + inputIndex + ")").val(towInput);
            } else if (index === 3 && inputIndex === 5) {
                towInput = $("#" + tabid + " .line:eq(3) input:eq(1)").val() + "L";
                $("#crons_set_table input:eq(" + inputIndex + ")").val(towInput);
        ...
        $("body").on("click", "#tab_w .line:eq(1) .bootstrap-touchspin-down," + "#tab_w .line:eq(1) .bootstrap-touchspin-up", function () {
            var tabid = $(this).closest(".tab-pane").attr("id");
            var findIndexObj = {
                tab_s: "0",
                tab_m: "1",
            ...
            var inputIndex = findIndexObj[tabid];
            if (getRadioIndex(tabid) === 1) {
                var towInput = $("#" + tabid + " .line:eq(1) input:eq(1)").val() + "/" + $("#" + tabid + " .line:eq(1) input:eq(2)").val();
                $("#crons_set_table input:eq(" + inputIndex + ")").val(towInput);
            }
            cornAciton();
        });
        ...
        // //周期 从星期 ---- （1表示星期天，2表示星期一， 依次类推）
        $("body").on("click", "#tab_w .line:eq(2) .bootstrap-touchspin-down," + "#tab_w .line:eq(2) .bootstrap-touchspin-up", function () {
            var tabid = $(this).closest(".tab-pane").attr("id");
            var findIndexObj = {
                tab_s: "0",
                tab_m: "1",
            ...
            if (getRadioIndex(tabid) === 2) {
                var towInput = $("#" + tabid + " .line:eq(2) input:eq(1)").val() + "#" + $("#" + tabid + " .line:eq(2) input:eq(2)").val();
                $("#crons_set_table input:eq(" + inputIndex + ")").val(towInput);
            }
            cornAciton();
        });
        ...
        $("body").on("click", "#tab_w .line:eq(3) .bootstrap-touchspin-down," + "#tab_w .line:eq(3) .bootstrap-touchspin-up", function () {
            var tabid = $(this).closest(".tab-pane").attr("id");
            var findIndexObj = {
                tab_s: "0",
                tab_m: "1",
            ...
            if (getRadioIndex(tabid) === 3) {
                var towInput = $("#" + tabid + " .line:eq(3) input:eq(1)").val() + "L";
                $("#crons_set_table input:eq(" + inputIndex + ")").val(towInput);
            }
            cornAciton();
        });
        ...
        $("body").on("click", "#tab_s .line:eq(2) .bootstrap-touchspin-down," + "#tab_s .line:eq(2) .bootstrap-touchspin-up," + "#tab_m .line:eq(2) .bootstrap-touchspin-down," + "#tab_m .line:eq(2) .bootstrap-touchspin-up," + "#tab_h .line:eq(2) .bootstrap-touchspin-down," + "#tab_h .line:eq(2) .bootstrap-touchspin-up," + "#tab_d .line:eq(2) .bootstrap-touchspin-down," + "#tab_d .line:eq(2) .bootstrap-touchspin-up," + "#tab_o .line:eq(2) .bootstrap-touchspin-down," + "#tab_o .line:eq(2) .bootstrap-touchspin-up", function () {
            var tabid = $(this).closest(".tab-pane").attr("id");
            var findIndexObj = {
                tab_s: "0",
                tab_m: "1",
        ...
        //指定具体秒
        $("body").on("click", "#tab_s .checkbox-div input,#tab_m .checkbox-div input,#tab_h .checkbox-div input,#tab_d .checkbox-div input,#tab_w .checkbox-div input,#tab_o .checkbox-div input", function () {
            var tabid = $(this).closest(".tab-pane").attr("id");
            var findIndexObj = {
                tab_s: "0",
                tab_m: "1",
        ...
        //日 每月 ... 号最近的那个工作日
        $("body").on("click", "#tab_d .line:eq(3) .bootstrap-touchspin-down,#tab_d .line:eq(3) .bootstrap-touchspin-up", function () {
            var tabid = $(this).closest(".tab-pane").attr("id");
            var findIndexObj = {
                tab_s: "0",
                tab_m: "1",
        ...
        function getSecondCheckBox(tabid) {
            ...
            $.each(checkEle, function (index, value) {
                if ($(value).find("span").attr("class") === "checked") {
        ...
        //作业树的处理
        //控制只准选择没有父级菜单的作业
        $("body").on("click", "#task-plan-setting [type=treeselectchild] .jstree-anchor", function () {
            if ($(this).closest("li").attr("class").indexOf("jstree-leaf") > -1) {
                var jobid = $(this).attr("id").match(/tbs-tree-(.*)_anchor/)[1];
                $(this).parents("[type=treeselectchild]").siblings("input[type=hidden]").val(jobid);
                $(this).parents("[type=treeselectchild]").siblings(".dropdown-toggle").val($(this).text()).attr("aria-expanded", "false").parent().removeClass("open");
            }
        });
