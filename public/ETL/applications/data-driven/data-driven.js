define(function (require, exports, module) {
  ...
  $(function () {
    ...
    $("body").on("click", "#dd-driven-add-modal", function () {
      ...
      $("#filename-add").remove();
      inputFileParent.append('<input id="dd-inputfile-add" type="file" name="file" style="display: none;">' + '<input type="button" id="filebutton-add" value="Select JAR">' + '<span id="filename-add">Please select a file</span>');
    ...
    $("body").on("click", "#data-driven-menu li", function () {
      ...
      $("#filebutton").remove();
      $("#filename").remove();
      inputFileParent.append('<input id="dd-inputfile" type="file" name="file" style="display: none;">' + '<input type="button" id="filebutton" value="Select JAR">' + '<span id="filename">Please select a file</span>');

      var _this = $(this);
      var _driverName = _this.find("strong").html();
      $(".r-container-header").find("h5").html(_driverName);
      ...
      $.each(showContent.list, function (i, item) {
        ...
        if (_driverName == item.driverName) {
          ...
          $("#driven-datatable-view input[name=driver_name]").val(item.driverName);
          $("#driven-datatable-view input[name=class_name]").val(item.className);
    ...
    $("body").on("click", "#driven_add_submit", function () {
      if ($("#driven-Add-Form").validate().form()) {
    ...
    $("body").on("change", "#dd-inputfile-add", function (data) {
      ...
      showContent.typeAdd = showContent.fileNameAdd.substring(showContent.fileNameAdd.length - 3, showContent.fileNameAdd.length);
    });
    ...
    $("body").on("click", "#data_Table02_Detail .delete-jar", function () {
      ...
      bootbox.confirm("Please confirm to delete.", function (result) {
        if (result) {
          var index = deletecIndex.parent("td").data("index");
    ...
    $("body").on("change", "#dd-inputfile", function (data) {
      ...
      showContent.type = showContent.fileName.substring(showContent.fileName.length - 3, showContent.fileName.length);
    });
    ...
    $("body").on("click", "#driven_Data_View_Detail .delete-jar", function () {
      ...
      bootbox.confirm("Please confirm to delete.", function (result) {
        if (result) {
          var index = deletecIndex.closest("td").data("index");
    ...
    $("body").on("click", "#dd-update-button", function (data) {
      if ($("#driven-datatable-view").validate().form()) {
