 ...
 */
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
	...
	var GridCoordMap = Class.extend({
		...
		constructor: function constructor(grid) {
			this.grid = grid;
		},
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
	...
	var ComboCoordMap = Class.extend({
		...
		constructor: function constructor(coordMaps) {
 ...
 */
	...
	var CellDragListener = DragListener.extend({
		...
		constructor: function constructor(coordMap, options) {
		...
		listenStart: function listenStart(ev) {
			...
			if (ev) {
				...
				if (subjectEl && this.options.subjectCenter) {
					...
					if (this.origCell) {
						subjectRect = intersectRects(this.origCell, subjectRect) || subjectRect; // in case there is no intersection
					}
		...
		getCell: function getCell(left, top) {
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
	...
	var RowRenderer = Class.extend({
		...
		rowHtml: function rowHtml(rowType, row) {
		...
		// `row` is an optional row number.
		bookendCells: function bookendCells(cells, rowType, row) {
		...
		getHtmlRenderer: function getHtmlRenderer(rendererName, rowType) {
			var view = this.view;
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		rangeToSegs: function rangeToSegs(range) {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		computeCellRange: function computeCellRange(cell) {
		...
		computeCellDate: function computeCellDate(cell) {
		...
		getRowEl: function getRowEl(row) {
			// subclasses should implement if leveraging the default getCellDayEl() or computeRowCoords()
		},

		// Retrieves the element representing the given column
		getColEl: function getColEl(col) {
			// subclasses should implement if leveraging the default getCellDayEl() or computeColCoords()
		},

		// Given a cell object, returns the element that represents the cell's whole-day
		getCellDayEl: function getCellDayEl(cell) {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		computeRowCoords: function computeRowCoords() {
			var items = [];
			var i, el;
			var top;
		...
		computeColCoords: function computeColCoords() {
			var items = [];
			var i, el;
			var left;
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		dayMousedown: function dayMousedown(ev) {
			...
			var dragListener = new CellDragListener(this.coordMap, {
				...
				cellOver: function cellOver(cell, isOrig, origCell) {
					if (origCell) {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		renderRangeHelper: function renderRangeHelper(range, sourceSeg) {
		...
		fabricateHelperEvent: function fabricateHelperEvent(range, sourceSeg) {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		computeSelection: function computeSelection(firstCell, lastCell) {
		...
		selectionRangeToSegs: function selectionRangeToSegs(range) {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		renderHighlight: function renderHighlight(segs) {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		// TODO: move to another class. not applicable to all Grids
		headCellHtml: function headCellHtml(cell) {
			var view = this.view;
			var date = cell.start;
		...
		bgCellHtml: function bgCellHtml(cell) {
			var view = this.view;
			var date = cell.start;
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		computeEventDrop: function computeEventDrop(startCell, endCell, event) {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		computeExternalDrop: function computeExternalDrop(cell, meta) {
			var dropLocation = {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		computeEventStartResize: function computeEventStartResize(startCell, endCell, event) {
			return this.computeEventResize('start', startCell, endCell, event);
		},

		// Returns new date-information for an event segment being resized from its end
		computeEventEndResize: function computeEventEndResize(startCell, endCell, event) {
		...
		computeEventResize: function computeEventResize(type, startCell, endCell, event) {
			var calendar = this.view.calendar;
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		eventsToSegs: function eventsToSegs(events, rangeToSegsFunc) {
		...
		// Doesn't guarantee an order for the resulting array.
		eventsToRanges: function eventsToRanges(events) {
			var _this = this;
		...
		eventsToNormalRanges: function eventsToNormalRanges(events) {
			var calendar = this.view.calendar;
			var ranges = [];
			var i, event;
		...
		eventsToInverseRanges: function eventsToInverseRanges(events) {
			var view = this.view;
		...
		eventRangeToSegs: function eventRangeToSegs(eventRange, rangeToSegsFunc) {
			var segs;
			var i, seg;
		...
		sortSegs: function sortSegs(segs) {
			segs.sort(proxy(this, 'compareSegs'));
		},

		// A cmp function for determining which segments should take visual priority
		// DOES NOT WORK ON INVERTED BACKGROUND EVENTS because they have no eventStartMS/eventDurationMS
		compareSegs: function compareSegs(seg1, seg2) {
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
	...
	var DayGrid = Grid.extend({
		...
		dayRowHtml: function dayRowHtml(row, isRigid) {
			var view = this.view;
		...
		dayCellHtml: function dayCellHtml(cell) {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		updateCellDates: function updateCellDates() {
			var view = this.view;
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		dateToCellOffset: function dateToCellOffset(date) {
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
	...
	DayGrid.mixin({
		...
		renderMoreLink: function renderMoreLink(cell, hiddenSegs) {
			var _this = this;
			var view = this.view;
		...
		showSegPopover: function showSegPopover(cell, moreLink, segs) {
			var _this = this;
			var view = this.view;
		...
		renderSegPopoverContent: function renderSegPopoverContent(cell, segs) {
			var view = this.view;
		...
		getCellSegs: function getCellSegs(cell, startLevel) {
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
	...
	var TimeGrid = Grid.extend({
		...
		slotBgCellHtml: function slotBgCellHtml(cell) {
			return this.bgCellHtml(cell);
		},

		// Generates the HTML for the horizontal "slats" that run width-wise. Has a time axis on a side. Depends on RTL.
		slatRowHtml: function slatRowHtml() {
			var view = this.view;
			var isRTL = this.isRTL;
			var html = '';
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		computeSnapTime: function computeSnapTime(row) {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		computeSlatTops: function computeSlatTops() {
			var tops = [];
			var top;
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
	...
	TimeGrid.mixin({
		...
		renderSegTable: function renderSegTable(segs) {
		...
		placeSlotSegs: function placeSlotSegs(segs) {
			var levels;
			var level0;
			var i;
		...
		computeSlotSegCoords: function computeSlotSegCoords(seg, seriesBackwardPressure, seriesBackwardCoord) {
		...
		updateSegVerticals: function updateSegVerticals() {
   ...
   */
		...
		generateSegPositionCss: function generateSegPositionCss(seg) {
		...
		groupSegCols: function groupSegCols(segs) {
			var segCols = [];
			var i;
		...
		sortForwardSlotSegs: function sortForwardSlotSegs(forwardSegs) {
			forwardSegs.sort(proxy(this, 'compareForwardSlotSegs'));
		},

		// A cmp function for determining which forward segment to rely on more when computing coordinates.
		compareForwardSlotSegs: function compareForwardSlotSegs(seg1, seg2) {
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		select: function select(range, ev) {
			this.unselect(ev);
		...
		reportSelection: function reportSelection(range, ev) {
			this.isSelected = true;
			this.triggerSelect(range, ev);
		},

		// Triggers handlers to 'select'
		triggerSelect: function triggerSelect(range, ev) {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		triggerDayClick: function triggerDayClick(cell, dayEl, ev) {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
	...
	var Calendar = fc.Calendar = Class.extend({
		...
		buildSelectRange: function buildSelectRange(start, end) {
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
	...
	var BasicView = View.extend({
		...
		headIntroHtml: function headIntroHtml() {
		...
		numberIntroHtml: function numberIntroHtml(row) {
			if (this.weekNumbersVisible) {
		...
		// Queried by the DayGrid subcomponent. Ordering depends on isRTL.
		dayIntroHtml: function dayIntroHtml() {
			if (this.weekNumbersVisible) {
		...
		numberCellHtml: function numberCellHtml(cell) {
			var date = cell.start;
			var classes;
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		renderHtml: function renderHtml() {
			...
			'</td>' + '</tr>' + '</thead>' + '<tbody class="fc-body">' + '<tr>' + '<td class="' + this.widgetContentClass + '">' + (this.dayGrid ? '<div class="fc-day-grid"/>' + '<hr class="fc-divider ' + this.widgetHeaderClass + '"/>' : '') + '<div class="fc-time-grid-container">' + '<div class="fc-time-grid"/>' + '</div>' + '</td>' + '</tr>' + '</tbody>' + '</table>';
		},
		...
		slotBgIntroHtml: function slotBgIntroHtml() {
