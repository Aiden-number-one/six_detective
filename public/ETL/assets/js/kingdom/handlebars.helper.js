...
//common
Handlebars.registerHelper("rowStyle", function (item) {
    // 表格奇偶行样式
...
Handlebars.registerHelper('foreach', function (context, options) {
    var ret = "";
    var context = (typeof context === "undefined" ? "undefined" : _typeof(context)) == "object" ? context : eval("(" + context + ")");
...
Handlebars.registerHelper("paymentmodeHelper", function (paymentmode) {
    if ("1" == paymentmode) {
        ...
        return "每日计息，按市值计算";
    } else if ("15" == paymentmode) {
        //13
        return "按月计息,按年付息.到期还本";
    } else if ("13" == paymentmode) {
        //13
        return "等额本息,按月还款";
    }
});
...
Handlebars.registerHelper("todolinkHelper", function (status) {
    switch (status) {
        ...
        case "audit_advanced_repay":
            ...
            break;
        case "audit_capitalregulate_apply":
            //资金调账审核
...
Handlebars.registerHelper("equalWithdraw", function (v1, v2, options) {
    //判断是否相等
    if (v1 == "提现失败" || v2 == "2") {
        return options.fn(this);
    } else {
...
//
Handlebars.registerHelper("getColor", function (v1) {
    //判断是否相等
    switch (v1) {
        case "1":
