...
define(function (require, exports, module) {
    ...
    $.extend({
        kingdom: {
            ...
            clearDateFormat: function clearDateFormat(fmt) {
            ...
            , uploadWithApi: function uploadWithApi(form_id, api, cbfunc) {
                var x_trace_user_id = $.kingdom.getValue("x-trace-user-id");
                if ($.trim(x_trace_user_id) == "") {
                    x_trace_user_id = $.kingdom.uuid();
                    $.kingdom.setValue("x-trace-user-id", x_trace_user_id);
            ...
            formatMillisecond: function formatMillisecond(s) {
                s = parseInt(s);
                if (isNaN(s)) {
                    return "--";
            ...
            */
            ...
            // 前端实现树搜索筛选数据
            filterFoldName: function filterFoldName(items, foldName) {
                var arr = [];
                ...
                try {
                    for (var _iterator = items[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                        ...
                        var temp = item.foldName.split("/");
                        for (var _i = 0, len = temp.length; _i < len; _i++) {
                            if (temp[_i].toLowerCase().includes(foldName.toLowerCase())) {
                                if (_i === 0) {
                                    arr = items;
                                    return arr;
                                } else if (_i === 1) {
                                    arr.push(item);
                                    var result = querySingle(items, arr, temp[0]);
                                    result && arr.push(result);
                                } else if (_i === 2) {
                                    arr.push(item);
                                    var _result = querySingle(items, arr, temp[0] + "/" + temp[1]);
                                    _result && arr.push(_result);
                                    var result2 = querySingle(items, arr, temp[0]);
                                    result2 && arr.push(result2);
                                } else {
                                    arr.push(item);
                                    var res = querySingle(items, arr, temp[0] + "/" + temp[1] + "/" + temp[2]);
                                    res && arr.push(res);
                                    var _result2 = querySingle(items, arr, temp[0] + "/" + temp[1]);
                                    _result2 && arr.push(_result2);
                                    var _result3 = querySingle(items, arr, temp[0]);
                                    _result3 && arr.push(_result3);
                                }
                            }
                        }
                    }
            ...
            // 普通列表查询封装, 避免搬砖时复制一大堆
            getList: function getList(obj, seccessCb) {
                var apiName = obj.apiName,
                    ...
                    cb = obj.cb,
                    isChangeSize = obj.isChangeSize,
                    isGoPage = obj.isGoPage,
                ...
                if (formName) {
                    paramsMap = _extends(App.getFormParams(formName), paramsMap);
                    //对搜索框中的日期格式进行处理
                    $.each($("form[name=" + formName + "] input.date-time-picker,form[name=" + formName + "] input.date-picker"), function (index, item) {
                ...
                $.kingdom.doKoauthAdminAPI(apiName, apiVision, paramsMap, function (data) {
                    ...
                    if (data.bcjson.flag == "0") {
                        ...
                        if (!items[0].hasOwnProperty("ROWNUM_")) {
                            var startPage = parseInt(paramsMap.pageSize) * parseInt(paramsMap.pageNumber); // 从哪开始计数，(防止这种情况：当输入pagenumber * pagisize > 数据库存的数量时 ，序号展示不对)
                            startPage = Math.floor(items.length / startPage) * parseInt(paramsMap.pageSize);
                            for (var _i2 = 0, len = items.length; _i2 < len; _i2++) {
                                items[_i2].ROWNUM_ = startPage + _i2 + 1;
                            }
                        }
            ...
            getDict: function getDict(dictType, selector, dictTypeName, template, resolve) {
                var dictData = sessionStorage.getItem("dictData");
                if (dictData) {
                    var parseDictData = JSON.parse(dictData);
                    var arr = [];
                    for (var _i3 in parseDictData[dictType]) {
                        var obj = {
                            id: _i3,
                            text: parseDictData[dictType][_i3],
                ...
                $.kingdom.doKoauthAdminAPI("bayconnect.superlop.get_all_dict_data_list", "v4.0", {}, function (data) {
                    if (data.bcjson.flag == "1") {
                        ...
                        var _arr = [];
                        for (var _i4 in items[0][dictType]) {
                            var _obj = {
                                id: _i4,
                                text: items[0][dictType][_i4],
                                dictTypeName: dictTypeName
                            };
                            _arr.push(_obj);
                        }
            ...
            getTreeData: function getTreeData() {
                ...
                this.GetData = function (root, arry, name, id, parentId, attrArray) {
                    ...
                    if (childArry.length > 0) {
                        ...
                        for (var i in childArry) {
                            var attr = "";
                            attrArray && attrArray.forEach(function (v) {
                                if (childArry[i][v]) {
                ...
                this.GetParentArry = function (id, parentId, arry) {
                    ...
                    for (var i in arry) {
                        if (arry[i][parentId] + '' === id) newArry.push(arry[i]);
                    }
                    return newArry;
                };
            },
            //防抖动
            debounce: function debounce(fn, delay, timer) {
                return function () {
                    var context = this;
                    var args = arguments;
                    clearTimeout(timer);
