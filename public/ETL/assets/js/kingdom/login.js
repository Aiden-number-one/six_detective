define(function (require, exports, module) {
    ...
    var Login = function () {
                ...
                success: function success(data) {
                    ...
                    if (data.bcjson.flag == "1" && data.bcjson.items) {
                        if (data.bcjson.items.length > 0) {
                            ...
                            var mounth = startTime.substring(4, 6);
                            var day = startTime.substring(6, 8);
                            var dateFormat = year + "-" + mounth + "-" + day;
        ...
        (function getAuthorizationDay() {
            $.kingdom.doKoauthAdminAPI(" bayconnect.superlop.get_sys_param_info", "v4.0", { "groupNo": "group.authinfo.warn", "paramKey": "group.authinfo.warn.deadline" }, function (data) {
        ...
        var buttonCountdown = function buttonCountdown($el, msNum, timeFormat) {
        ...
        var getcheckcode = function getcheckcode(paramsMap) {
        ...
        //获取短信验证码
        var messagecode = function messagecode(value, element) {
        ...
        //恢复发送验证码按钮
        var leftcodetime = function leftcodetime(time) {
            if (time == 0 || time < 0) {
        ...
        //获取验证码图片
        var validateimg = function validateimg(value, element) {
            var result = false;
